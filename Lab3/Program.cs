...
app.MapGet("/api/groups", [Authorize] async (ModelDB db) => await db.PriceLists!.ToListAsync());
app.MapGet("api/group/{id:int}", [Authorize] async (ModelDB db, int id) => await db.PriceLists!.Where(g => g.Id == id).FirstOrDefaultAsync());
app.MapGet("/api/students", [Authorize] async (ModelDB db) => await db.products!.ToListAsync());
app.MapGet("/api/group/{name}", [Authorize] async (ModelDB db,double idd ) => await db.products!.Where(u=>u.Id==idd).FirstOrDefaultAsync());
app.MapPost("/api/group", [Authorize] async (PriceList priceList, ModelDB db) =>
{
...
{
    PriceList? PriceList = await db.PriceLists!.FirstOrDefaultAsync(u => u.Id == id);
    if (PriceList == null) return Results.NotFound(new { message = "Студент не найден" });
    db.PriceLists!.Remove(PriceList);
    await db.SaveChangesAsync();
    return Results.Json(PriceList);
});
...
app.MapPut("/api/student", [Authorize] async (Product product, ModelDB db) =>
{
